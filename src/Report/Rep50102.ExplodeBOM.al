report 50102 ExplodeBOM
{
    ...
    var
        ...
        ExpBOMLineTMP: Record ExpBOMLine temporary;

    local procedure FillBOMTable()
    begin
        ExpBOMLineTMP.LineNo += 1;
        ExpBOMLineTMP.ItemNo := Item."No.";
        ExpBOMLineTMP.Level := Level;
        ExpBOMLineTMP.CompNo := BomComponent[Level]."No.";
        ExpBOMLineTMP.CompVar := BomComponent[Level]."Variant Code";
        ExpBOMLineTMP.CompDesc := BomComponent[Level].Description;
        ExpBOMLineTMP.BOMQuantity := BOMQty;
        ExpBOMLineTMP.UoM := BomComponent[Level]."Unit of Measure Code";
        ExpBOMLineTMP.TotalQty := BomComponent[Level].Quantity;
        ExpBOMLineTMP.Insert();
    end;

    procedure getBOMTable(var vExpBOMLineTMP: Record ExpBOMLine)
    begin
        Clear(vExpBOMLineTMP);
        vExpBOMLineTMP.Copy(ExpBOMLineTMP, true);
    end;

    procedure SetBOMVers(iBOMVers: Code[20])
    begin
        BOMVersion := iBOMVers;
    end;
